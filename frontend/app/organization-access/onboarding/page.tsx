...
type OrganizationAccessOnboardingResponse = {
  status: string;
  message: string;
};
  ...
  if (Array.isArray(detail)) {
    return detail
      .map((item) => {
        ...
        return "Invalid organisation access request.";
      })
      .join(" ");
  }

  return "Unable to complete account setup.";
}

export default function OrganizationAccessOnboardingPage() {
  ...
  async function handleSubmit(
    ...
    if (!token) {
      setErrorMessage(
        "The organisation access invitation is invalid because no token was provided."
      );
      return;
    }
    ...
    try {
      ...
      const response =
        await fetch(
          `${API_URL}/api/v1/auth/organization-access/onboarding`,
          {
            method: "POST",
      ...
      const result =
        data as OrganizationAccessOnboardingResponse;
      ...
      console.error(
        "Organisation access onboarding error:",
        error
      );
      ...
      >
        ...
        >
          ...
          {!token && (
            ...
            >
              This organisation access invitation
